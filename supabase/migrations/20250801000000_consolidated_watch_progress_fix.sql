-- This migration consolidates previous attempts to fix the save_watch_progress functionality.
-- It ensures that the correct partial unique indexes and the corresponding function logic are in place,
-- resolving "ON CONFLICT" errors by aligning the function with the schema.

-- Step 1: Drop any legacy unique constraints that might conflict.
-- These were the old, non-partial constraints that caused issues with NULLs.
...
-- This creates separate uniqueness rules for movies (episode_number IS NULL)
...
-- based on whether an episode number is present.
...
-- Step 4: Grant permissions to ensure the function can be executed by authenticated users.
